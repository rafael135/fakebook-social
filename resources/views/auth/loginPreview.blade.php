...
@section("content")
    ...
    <div class="pt-16 w-full min-h-screen flex md:flex-col lg:flex-row">
        <div class="form-container flex-1 h-auto bg-gradient-to-t from-gray-100 from-70% to-gray-200 overflow-hidden border-solid border-b-gray-400/80 shadow-lg shadow-black/20">
            ...
            <form class="px-4 h-full py-4 flex-1 flex flex-col justify-center gap-2" method="POST" action='{{route("auth.loginAction")}}'>
                @csrf
                ...
                <input class="auth-input self-center @if($passwordError != null) error @endif" type="password" autocomplete="current-password" name="password" id="password" placeholder="@if($passwordError == null) Senha @else {{$passwordError}} @endif" data-input-placeholder="Senha">
